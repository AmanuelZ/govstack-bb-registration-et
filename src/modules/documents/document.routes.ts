...
export async function documentRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.post('/documents', {
    schema: {
      ...
      description: 'Upload a supporting document for an application. Accepts PDF and image files up to the configured size limit.',
      ...
      body: {
        ...
        properties: {
          ...
          documentType: { type: 'string', description: 'e.g. MOA_AOA, TAX_CLEARANCE, BANK_LETTER' },
    ...
    if (!ALLOWED_MIME_TYPES.has(mimetype)) {
      throw AppError.badRequest(`File type '${mimetype}' not allowed. Accepted: PDF, JPEG, PNG, WebP`);
    }
