...
export async function eFormRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/services/:serviceId/eForms', {
    schema: {
      tags: ['eForms'],
  ...
  fastify.get('/eForms/:eFormId', {
    schema: {
      tags: ['eForms'],
      summary: 'Get e-form schema by ID',
      description: 'Returns the full dynamic e-form schema including field definitions, validation rules, and UI rendering hints. Bilingual (English + Amharic).',
      params: {
        type: 'object',
