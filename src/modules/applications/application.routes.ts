...
export async function applicationRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.post('/services/:serviceId/applications', {
    schema: {
      tags: ['Applications'],
      summary: 'Submit a new application',
      description: 'Submit a new registration application for a service. Creates the application and generates the first back-office task.',
      params: {
        type: 'object',
      ...
      body: {
        ...
        required: ['formData'],
        properties: {
          formData: { type: 'object', additionalProperties: true, description: 'Form field values matching the service e-form schema' },
  ...
  fastify.get('/applications/:fileId', {
    schema: {
      tags: ['Applications'],
  ...
  fastify.put('/applications/:fileId', {
    schema: {
      tags: ['Applications'],
      summary: 'Update application',
      description: 'Applicants can update form data on DRAFT/SENT_BACK applications or withdraw any non-terminal application.',
      params: {
        type: 'object',
        properties: { fileId: { type: 'string', format: 'uuid' } },
        required: ['fileId'],
