...
export async function statisticsRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.get('/data/statistics', {
    schema: {
      tags: ['Statistics'],
      summary: 'Get operational statistics',
      description: 'Returns aggregate statistics on applications: counts by status, average processing times, and service breakdown.',
      querystring: {
        type: 'object',
        properties: {
    ...
    const [totalApplications, applicationsByStatus, applicationsByService, totalTasks, pendingTasks, overdueTasks] =
      await Promise.all([
