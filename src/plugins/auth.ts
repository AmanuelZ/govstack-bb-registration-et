...
export async function authRoutes(fastify: FastifyInstance): Promise<void> {
    ...
    schema: {
      ...
      querystring: {
        ...
        properties: {
          login_hint: {
            type: 'string',
            description: 'Development only: hint for mock provider (test_applicant, test_name_reviewer, etc.)',
          },
        },
      },
    },
  ...
  fastify.get('/auth/callback', {
    schema: {
      ...
      description: 'Receives the authorization code from Fayda eSignet, exchanges it for tokens, and issues a session JWT.',
      querystring: {
        type: 'object',
        properties: {
          code: { type: 'string' },
    ...
    if (query['error']) {
      logger.warn({ error: query['error'], description: query['error_description'] }, 'Fayda auth error');
